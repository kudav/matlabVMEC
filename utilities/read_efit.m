...
%READ_EFIT Reads files generated by an EFIT (g-file,a-file)
%   The READ_EFIT routine takes a filename as input and attempts to read
%   and return the result in a data structure.  It currently supports
%   reading of g-filess and a-files.
%
%   Example:
%       data=read_efit('g30841.03901');
%
...
switch filename_short(1)
    ...
    otherwise
        if strcmp(filename_short(end-4:end),'eqdsk') || strcmp(filename_short(end-5:end-1),'eqdsk') || strcmp(filename_short(end-6:end-2),'eqdsk') || strcmp(filename_short(1:3),'mic')
            filetype='gfile';
        else
...
switch filetype
    case 'gfile'
        ...
        % Use regexp to extract the last three integer numbers
        numbers = regexp(header, '\d+', 'match'); % Extract all numbers as strings
        temp = str2double(numbers(end-2:end)); % Convert last three to numeric array
...
%         nz=header2{7};
        ...
        efit_data.xsep=fscanf(fid,'%e',1);
        efit_data.zsep=fscanf(fid,'%e',1);
        efit_data.sf=fscanf(fid,'%e',nx); %Poloidal flux profile (?)
        efit_data.sp=fscanf(fid,'%e',nx); %Pressure profiles
        efit_data.sffp=fscanf(fid,'%e',nx); %Gradient of poloidal flux
        efit_data.spp=fscanf(fid,'%e',nx); %Pressure gradient
        efit_data.psixz=fscanf(fid,'%e',[nx nz]); %Normalized poloidal flux
        efit_data.qpsi=fscanf(fid,'%e',nx); %Q profile
        efit_data.nbndry=fscanf(fid,'%e',1);
        efit_data.nlim=fscanf(fid,'%e',1);
        bdry_temp=fscanf(fid,'%e',efit_data.nbndry*2);
        efit_data.xbndry=bdry_temp(1:2:efit_data.nbndry*2);
